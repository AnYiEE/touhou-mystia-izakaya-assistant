...
| P5   | 初版未注册用户仍走口令注册                | 历史   | 已被第十五节覆盖：用户明确确认后可用通行密钥创建账号，随后再设置初始密码                               |
...
初版非目标中的“注册即创建通行密钥”已由第十五节后续增量覆盖；仍不纳入永久 passkey-only 账号、跨设备 QR 自定义引导、MFA 分级和 attestation 厂商白名单。
...
- 通行密钥作为永久唯一凭证仍不纳入；“注册阶段即创建通行密钥”已由第十五节增量实现，并要求用户后续可设置口令兜底。
