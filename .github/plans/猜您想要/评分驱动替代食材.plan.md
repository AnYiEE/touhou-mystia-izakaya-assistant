...
**文件**：历史上曾计划新建 `app/utils/customer/customer_rare/getScoreBasedAlternatives.ts`，后来合并到当时的 `suggestMeals.ts`；当前领域内核位于 [`app/domain/recommendations/suggestMeals.ts`](../../../app/domain/recommendations/suggestMeals.ts)，客户端调度入口位于 [`app/features/recommendations/client/suggestMeals.ts`](../../../app/features/recommendations/client/suggestMeals.ts)。
...
**影响范围（以下旧路径是当时的迁移记录，不是当前 import 指引）**：
