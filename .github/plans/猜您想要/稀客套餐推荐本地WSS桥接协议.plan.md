...
| `ready`         | 收到 `bridge.ready`                                            | 接受推荐和取消消息。                          |
...
没有有效启动描述时 `startRecommendationBridgeClient()` 不创建 runtime，因此没有单独的 `idle` 运行时状态。
...
`extra_ingredient_ids` 在业务上是无序多重集合：数组顺序不表示食材之间的推荐优先级，同一 ID 可以出现多次，每次出现都表示一个实际食材槽位。Mod 比较套餐身份时必须包含 `recipe_id`，并保留每个额外食材 ID 的出现次数，但不能依赖这些 ID 的排列顺序。
...
- 改变 `max_rating` 硬约束、第一项为综合权重首选、后续项保持评级优先、结果无重复、取消不产生晚到结果以及断线不自动重放等可观察语义。
...
推荐结果不按协议版本冻结为一组永久不变的套餐。相同请求在不同网页发布版本中可以因游戏数据、算法正确性修复或结果选择优化产生不同结果，但仍必须满足 V1 规定的硬约束：第一项是综合权重首选，后续项不跨评级提升，并保持结果结构和确定性。`client.version` 用于记录产生结果的网页版本，不是要求 Mod 匹配的最低版本。
