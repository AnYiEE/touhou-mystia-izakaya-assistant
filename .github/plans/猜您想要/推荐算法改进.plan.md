...
> 历史定位：本文记录 Beam Search 阶段的改进和当时的实施结果。当前生产搜索已由 [精确异步推荐算法](精确异步推荐算法.plan.md) 删除 `BEAM_WIDTH`、`tryAddExtraIngredients` 和提前终止逻辑，改为精确状态动态规划、异步时间片与分层有界缓存。本文的 Beam 伪代码、同步性能估计和 `CACHE_MAX_SIZE=200` 仅用于解释历史决策。
