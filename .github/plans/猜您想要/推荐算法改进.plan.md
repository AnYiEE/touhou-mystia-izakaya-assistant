...
>
> **影响**：会改变部分「同评分内」候选的基础顺序（属有意改进，非 bug），评分等级层级与现有结果完全一致。当前唯一入口为 `selectScoredResults`：它先使用归一化比较器排序并精确去重，再按 [`推荐结果多样化.plan.md`](.github/plans/猜你想要/推荐结果多样化.plan.md) 选择最终结果。缓存键无需变更（指标由相同输入派生）。
>
