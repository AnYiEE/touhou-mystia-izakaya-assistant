...
- **模式 A** `suggestForRecipe`：固定已选料理，遍历可用酒水，对每个酒水调用 `evaluateMeal`（传入 `hasMystiaCooker`），同时尝试添加额外食材提升评分。候选完成统一排序和精确去重后，第一条保留综合权重第一名，后续在同评级内优先选择不同酒水，最多返回 N 个 `ISuggestedMeal`。
