...
模式 A、B 和 D 共用 [`推荐结果多样化.plan.md`](.github/plans/猜你想要/推荐结果多样化.plan.md) 中的最终结果选择规则。固定料理的模式 A 只分散酒水，固定酒水的模式 B 只分散料理；模式 D 同时考虑料理和酒水。模式 C 仍最多返回一条改进结果。`maxResults` 只限制最终输出数量，不限制选择器检查的完整候选范围。
