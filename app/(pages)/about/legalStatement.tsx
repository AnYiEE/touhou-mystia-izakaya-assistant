	...
	return (
		<>
			...
			<div className="space-y-2 break-all text-justify indent-8">
				...
				<p>
					<strong>账号注册与个人信息收集</strong>
				</p>
				<p>
					当您注册{shortName}
					账号时，您需要提供用户名和密码。用户名将会公开显示，密码在服务器端仅存储其不可逆的哈希摘要。此外，系统会在您登录时自动生成会话标识，并记录账号的创建时间、最近登录时间、登录失败次数和账号状态等必要信息。上述信息的收集基于您自愿注册账号的同意行为，且为实现账号服务功能（包括身份验证、会话管理、安全防护和密码找回等）所必需。
				</p>
				<p>
					<strong>账号安全与用户义务</strong>
				</p>
				<p>
				...
				<p>
					<strong>Cookie与会话</strong>
				</p>
				<p>
					{shortName}
					使用必要的会话Cookie来维持您的登录状态和提供SSO授权流程中的安全上下文传递。这些Cookie为功能必需项，不用于广告追踪或行为分析目的。会话Cookie在您退出登录或关闭浏览器后失效（受限于浏览器的实际行为）。SSO相关的临时上下文Cookie设置了较短的过期时间，并在授权完成后自动清除。
				</p>
				<p>
					<strong>未成年人保护</strong>
				</p>
				<p>
					{shortName}
				...
				<p>本网站仅作为个人兴趣项目，不提供任何有偿服务或附加权益。</p>
			</div>
		</>
	);
}
