import { type Metadata } from 'next';

import { BadgeCatalog } from '@/domain/catalog/items/BadgeCatalog';

import { getPageTitle } from '@/features/appShell/navigation/getPageTitle';

import { SITE_METADATA } from '@/shared/site/metadata';

const { description, keywords } = SITE_METADATA;

const badges = BadgeCatalog.getInstance().getNames(10);
const title = getPageTitle('/badges');

export const metadata: Metadata = {
	title,

	description: `本页面可以查询${badges.join('、')}等${title}的详情。${description}`,
	keywords: keywords.toSpliced(18, Infinity, ...badges),
};

export { default } from '@/features/preferences/client/components/PreferencesModalLayout';
