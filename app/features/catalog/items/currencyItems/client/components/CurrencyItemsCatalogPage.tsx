...
export default function CurrencyItemsCatalogPage() {
	...
	const availableAvailabilityDlcs =
		currencyItemsStore.availableAvailabilityDlcs.use();
	const availableContentDlcs = currencyItemsStore.availableContentDlcs.use();

	const pinyinSortState =
		currencyItemsStore.persistence.pinyinSortState.use();

	const filterAvailabilityDlcs =
		currencyItemsStore.persistence.filters.availabilityDlcs.use();
	const filterContentDlcs =
		currencyItemsStore.persistence.filters.contentDlcs.use();

	const filterData = useCallback(
		() =>
			filterCurrencyItemData({
				data: instance.data,
	...
	const pinyinSortConfig = useMemo<IPinyinSortConfig>(
		() => ({
			pinyinSortState,
			setPinyinSortState:
				currencyItemsStore.persistence.pinyinSortState.set,
		}),
		[pinyinSortState]
	);
	...
	return (
		...
		>
			<CurrencyItemsCatalog data={sortedData} />
		</ItemPage>
	);
}
