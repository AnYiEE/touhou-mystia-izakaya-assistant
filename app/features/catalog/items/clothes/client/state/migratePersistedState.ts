export const CLOTHES_STORE_VERSION = {
	initial: 0,
	removeSearchValue: 1, // eslint-disable-next-line sort-keys
	availabilityDlcFilter: 2,
} as const;

export function migrateClothesPersistedState<T>(
	persistedState: T,
	version: number
): T {
	if (version >= CLOTHES_STORE_VERSION.availabilityDlcFilter) {
		return persistedState;
	}
	// eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
	const oldState = structuredClone(persistedState) as any;
	if (version < CLOTHES_STORE_VERSION.removeSearchValue) {
