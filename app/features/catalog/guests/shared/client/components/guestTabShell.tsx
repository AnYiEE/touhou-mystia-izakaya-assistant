...
interface IProps<TItemId extends Key, TItemName extends string> {
	...
	isVisible: boolean;
	onSelect: (id: TItemId, name: TItemName) => void;
	onToggleVisibility: () => void;
	renderAvatar: (id: TItemId, name: TItemName) => ReactNode;
	sortedData: ReadonlyArray<{ id: TItemId; name: TItemName }>;
}

export default memo(function GuestTabShell<
	TItemId extends Key,
...
>({
	...
	const updateViewportHeight = useCallback(() => {
		...
		const { maxHeight, minHeight } = getComputedStyle(constraintElement);
	...
	useLayoutEffect(() => {
		...
		cancelViewportHeightUpdate,
		guestTabStyle.classNames.content,
		isVisible,
		scheduleViewportHeightUpdate,
		updateViewportHeight,
	]);
...
}) as <TItemId extends Key, TItemName extends string>(
	props: IProps<TItemId, TItemName>
) => ReactNode;
