...
export default function FoodTabContent() {
	...
	const selectedCookerTypes = normalGuestStore.foodTableCookerTypes.use();
	const selectedAvailabilityDlcs =
		normalGuestStore.foodTableAvailabilityDlcs.use();

	const availableFoodCookers = normalGuestStore.availableFoodCookers.use();
	const availableFoodAvailabilityDlcs =
		normalGuestStore.availableFoodAvailabilityDlcs.use();
	const availableFoods = normalGuestStore.availableFoods.use();
	const availableFoodTags = normalGuestStore.availableFoodTags.use();

	const searchValue = normalGuestStore.shared.recipe.searchValue.use();

	const tableCurrentPage = normalGuestStore.shared.recipe.table.page.use();
	const tableRowsPerPage = normalGuestStore.shared.recipe.table.rows.use();
	const tableSelectableRows =
		normalGuestStore.shared.recipe.table.selectableRows.get();
	const tableSortDescriptor =
		normalGuestStore.persistence.recipe.table.sortDescriptor.use();
	const tableVisibleColumns =
		normalGuestStore.shared.recipe.table.columns.use();

	const {
		pagedRows: tableCurrentPageItems,
		sortedRows: tableSortedRows,
		totalPages: tableTotalPages,
	} = normalGuestStore.foodTableRows.use();
	...
	const renderTableCell = useCallback(
		(foodRow: TFoodSuitabilityRow, columnKey: TFoodTableColumnKey) => {
			...
			const { positive: positiveTagStyle } = NORMAL_GUEST_TAG_STYLE;
				...
				case 'action':
					return (
						<FoodTableActionButton
							...
							onSelect={() => {
								vibrate();
								normalGuestStore.onFoodTableAction(
									food,
									recipeId
								);
							}}
				...
				<div className="flex flex-col justify-between gap-x-3 gap-y-2 md:flex-row">
					<div className="flex flex-1 items-end gap-3">
						<Autocomplete
							...
							onInputChange={(value) => {
								vibrate(!value);
								normalGuestStore.onFoodTableSearchValueChange(
									value
								);
							}}
	...
	const handleTablePageChange = useCallback(
		(page: number) => {
			vibrate();
			normalGuestStore.onFoodTablePageChange(page);
		},
		[vibrate]
	);
	const handleTableSortChange = useCallback(
		(config: SortDescriptor) => {
			vibrate();
			normalGuestStore.onFoodTableSortChange(
