...
export default function BeverageTabContent() {
	...
	const selectedAvailabilityDlcs =
		normalGuestStore.beverageTableAvailabilityDlcs.use();

	const availableBeverageAvailabilityDlcs =
		normalGuestStore.availableBeverageAvailabilityDlcs.use();
	const availableBeverages = normalGuestStore.availableBeverages.use();
	const availableBeverageTags = normalGuestStore.availableBeverageTags.use();

	const searchValue = normalGuestStore.shared.beverage.searchValue.use();

	const tableCurrentPage = normalGuestStore.shared.beverage.table.page.use();
	const tableRowsPerPage = normalGuestStore.shared.beverage.table.rows.use();
	const tableSelectableRows =
		normalGuestStore.shared.beverage.table.selectableRows.get();
	const tableSortDescriptor =
		normalGuestStore.persistence.beverage.table.sortDescriptor.use();
	const tableVisibleColumns =
		normalGuestStore.shared.beverage.table.columns.use();

	const {
		pagedRows: tableCurrentPageItems,
		sortedRows: tableSortedRows,
		totalPages: tableTotalPages,
	} = normalGuestStore.beverageTableRows.use();
	...
	const renderTableCell = useCallback(
		(
			...
			const { beverage: beverageTagStyle } = NORMAL_GUEST_TAG_STYLE;
						...
						<div className="flex justify-center">
							...
							>
								<Button
									...
									onPress={() => {
										vibrate();
										normalGuestStore.onBeverageTableAction(
											id
										);
									}}
									aria-label={label}
		...
		[currentNormalGuest, openWindow, tableVisibleColumns.size, vibrate]
	);
				...
				<div className="flex flex-col justify-between gap-x-3 gap-y-2 md:flex-row">
					<div className="flex flex-1 items-end gap-3">
						<Autocomplete
							...
							onInputChange={(value) => {
								vibrate(!value);
								normalGuestStore.onBeverageTableSearchValueChange(
									value
								);
							}}
						...
						<Select
							...
							variant="flat"
							onSelectionChange={
								normalGuestStore.onBeverageTableSelectedTagsChange
							}
						...
						{availableBeverageAvailabilityDlcs.length > 1 && (
							<Dropdown showArrow>
								...
								<DropdownMenu
									...
									variant="flat"
									onSelectionChange={
										normalGuestStore.onBeverageTableSelectedAvailabilityDlcsChange
									}
	...
	const handleTablePageChange = useCallback(
		(page: number) => {
			vibrate();
			normalGuestStore.onBeverageTablePageChange(page);
		},
		[vibrate]
	);
	const handleTableSortChange = useCallback(
		(config: SortDescriptor) => {
			vibrate();
			normalGuestStore.onBeverageTableSortChange(
