...
import BeverageTableShell from '@/features/catalog/customers/shared/client/components/beverageTableShell';
import TagGroup from '@/features/catalog/customers/shared/client/components/tagGroup';
import type { TBeverageSuitabilityRow } from '@/features/catalog/customers/shared/contracts';
import {
...
import { CUSTOMER_RARE_TAG_STYLE } from '@/features/catalog/presentation/tagStyles';
...
export default function BeverageTabContent() {
	...
	const selectedCustomerBeverageTag =
		customerRareStore.shared.customer.select.beverageTag.use();

	const currentBeverageName = customerRareStore.shared.beverage.name.use();
	const selectedAvailabilityDlcs =
		customerRareStore.beverageTableAvailabilityDlcs.use();

	const availableBeverageAvailabilityDlcs =
		customerRareStore.availableBeverageAvailabilityDlcs.use();
	const availableBeverageNames =
		customerRareStore.availableBeverageNames.use();
	const availableBeverageTags = customerRareStore.availableBeverageTags.use();

	const searchValue = customerRareStore.shared.beverage.searchValue.use();

	const tableCurrentPage = customerRareStore.shared.beverage.table.page.use();
	const tableRowsPerPage = customerRareStore.shared.beverage.table.rows.use();
	const tableSelectableRows =
		customerRareStore.shared.beverage.table.selectableRows.get();
	const tableSortDescriptor =
		customerRareStore.persistence.beverage.table.sortDescriptor.use();
	const tableVisibleColumns =
		customerRareStore.shared.beverage.table.columns.use();

	const {
		pagedRows: tableCurrentPageItems,
		sortedRows: tableSortedRows,
		totalPages: tableTotalPages,
	} = customerRareStore.beverageTableRows.use();
						...
						<div className="flex justify-center">
							...
							>
								<Button
									...
									onPress={() => {
										vibrate();
										customerRareStore.onBeverageTableAction(
											name
										);
									}}
									aria-label={label}
				...
				<div className="flex flex-col justify-between gap-x-3 gap-y-2 md:flex-row">
					<div className="flex flex-1 items-end gap-3">
						<Autocomplete
							...
							onInputChange={(value) => {
								vibrate(!value);
								customerRareStore.onBeverageTableSearchValueChange(
									value
								);
							}}
						...
						<Select
							...
							variant="flat"
							onSelectionChange={
								customerRareStore.onBeverageTableSelectedTagsChange
							}
						...
						{availableBeverageAvailabilityDlcs.length > 1 && (
							<Dropdown showArrow>
								...
								<DropdownMenu
									...
									variant="flat"
									onSelectionChange={
										customerRareStore.onBeverageTableSelectedAvailabilityDlcsChange
									}
	...
	return (
		<BeverageTableShell
			...
			onPageChange={(page) => {
				vibrate();
				customerRareStore.onBeverageTablePageChange(page);
			}}
			onSortChange={(config) => {
				vibrate();
				customerRareStore.onBeverageTableSortChange(
