...
import { buildRareTagTooltip } from '@/features/catalog/customers/shared/presentation/buildTagTooltip';
import { getCustomerRareDisplayMeta } from '@/features/catalog/presentation/customerDisplayMeta';
...
export default function CustomerCard() {
	...
	const currentRecipeTagsWithTrend =
		customerRareStore.shared.recipe.tagsWithTrend.use();
	...
	const handleBeverageTagPress = useCallback(
		(tag: TBeverageTag) => {
			vibrate();
			customerRareStore.onCustomerOrderBeverageTag(tag);
			if (isOrderLinkedFilter) {
				customerRareStore.onCustomerFilterBeverageTag(
					tag,
					hasMystiaCooker
				);
			}
	...
	const handleRecipeTagPress = useCallback(
		(tag: TRecipeTag) => {
			vibrate();
			customerRareStore.onCustomerOrderRecipeTag(tag);
			if (isOrderLinkedFilter) {
				customerRareStore.onCustomerFilterRecipeTag(
					tag,
					hasMystiaCooker
				);
			}
	...
	const handleRefreshCustomer = useCallback(() => {
		vibrate();
		customerRareStore.onCustomerSelectedChange(null);
		pushState('/customer-rare');
	}, [pushState, vibrate]);
	...
	const {
		...
		placeContent,
	} = getCustomerRareDisplayMeta(instance_customer, currentCustomerName);
