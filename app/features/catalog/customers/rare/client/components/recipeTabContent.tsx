...
export default function RecipeTabContent() {
	...
	const selectedCustomerRecipeTag =
		customerRareStore.shared.customer.select.recipeTag.use();

	const currentRecipeData = customerRareStore.shared.recipe.data.use();
	const selectedCookers = customerRareStore.recipeTableCookers.use();
	const selectedAvailabilityDlcs =
		customerRareStore.recipeTableAvailabilityDlcs.use();

	const availableRecipeCookers =
		customerRareStore.availableRecipeCookers.use();
	const availableRecipeAvailabilityDlcs =
		customerRareStore.availableRecipeAvailabilityDlcs.use();
	const availableRecipeNames = customerRareStore.availableRecipeNames.use();
	const availableRecipeTags = customerRareStore.availableRecipeTags.use();

	const searchValue = customerRareStore.shared.recipe.searchValue.use();

	const tableCurrentPage = customerRareStore.shared.recipe.table.page.use();
	const tableRowsPerPage = customerRareStore.shared.recipe.table.rows.use();
	const tableSelectableRows =
		customerRareStore.shared.recipe.table.selectableRows.get();
	const tableSortDescriptor =
		customerRareStore.persistence.recipe.table.sortDescriptor.use();
	const tableVisibleColumns =
		customerRareStore.shared.recipe.table.columns.use();

	const {
		pagedRows: tableCurrentPageItems,
		sortedRows: tableSortedRows,
		totalPages: tableTotalPages,
	} = customerRareStore.recipeTableRows.use();
				...
				case 'action':
					return (
						<RecipeTableActionButton
							...
							onSelect={() => {
								vibrate();
								customerRareStore.onRecipeTableAction(name);
							}}
						/>
					);
			}
				...
				<div className="flex flex-col justify-between gap-x-3 gap-y-2 md:flex-row">
					<div className="flex flex-1 items-end gap-3">
						<Autocomplete
							...
							onInputChange={(value) => {
								vibrate(!value);
								customerRareStore.onRecipeTableSearchValueChange(
									value
								);
							}}
						...
						<Select
							...
							variant="flat"
							onSelectionChange={
								customerRareStore.onRecipeTableSelectedPositiveTagsChange
							}
					...
					<div className="flex w-full gap-3 md:w-auto">
						<Dropdown showArrow>
							...
							<DropdownMenu
								...
								variant="flat"
								onSelectionChange={
									customerRareStore.onRecipeTableSelectedCookersChange
								}
						...
						{availableRecipeAvailabilityDlcs.length > 1 && (
							<Dropdown showArrow>
								...
								<DropdownMenu
									...
									variant="flat"
									onSelectionChange={
										customerRareStore.onRecipeTableSelectedAvailabilityDlcsChange
									}
	...
	return (
		<RecipeTableShell
			...
			onPageChange={(page) => {
				vibrate();
				customerRareStore.onRecipeTablePageChange(page);
			}}
			onSortChange={(config) => {
				vibrate();
				customerRareStore.onRecipeTableSortChange(
