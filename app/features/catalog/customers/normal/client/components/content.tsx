...
export default function Content() {
	...
	useEffect(() => {
		customerNormalStore.shared.customer.name.set(validName);
	}, [validName]);
	...
	const availableCustomerAvailabilityDlcs =
		customerNormalStore.availableCustomerAvailabilityDlcs.use();
	const availableCustomerNames =
		customerNormalStore.availableCustomerNames.use();
	const availableCustomerPlaces =
		customerNormalStore.availableCustomerPlaces.use();

	const customerPinyinSortState =
		customerNormalStore.persistence.customer.pinyinSortState.use();

	const customerFilterAvailabilityDlcs =
		customerNormalStore.persistence.customer.filters.availabilityDlcs.use();
	const customerFilterPlaces =
		customerNormalStore.persistence.customer.filters.places.use();
	const customerFilterNoPlaces =
		customerNormalStore.persistence.customer.filters.noPlaces.use();
	const customerFilterIncludes =
		customerNormalStore.persistence.customer.filters.includes.use();
	const customerFilterExcludes =
		customerNormalStore.persistence.customer.filters.excludes.use();

	const customerPinyinSortConfig = useMemo<IPinyinSortConfig>(
		() => ({
			pinyinSortState: customerPinyinSortState,
			setPinyinSortState:
				customerNormalStore.persistence.customer.pinyinSortState.set,
		}),
		...
		() => [
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.customer.filters
			...
			},
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.customer.filters.places.set,
			},
			{
				items: availableCustomerPlaces,
				label: '出没地区（排除）',
				...
				setSelectedKeys:
					customerNormalStore.persistence.customer.filters.noPlaces
						.set,
			},
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.customer.filters.includes
						.set,
			...
			},
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.customer.filters.excludes
						.set,
				spriteTarget: 'customer_normal',
			},
		],
	...
	const customerTabVisibilityState =
		customerNormalStore.persistence.customer.tabVisibility.use();

	const customerTabStyle = customerTabStyleMap[customerTabVisibilityState];

	const isCustomerTabFilterVisible =
		customerNormalStore.shared.customer.filterVisibility.use();

	const { ingredientFilteredData, ingredientSortedData } =
		useIngredientRouteData(customerNormalStore);

	const availableIngredientAvailabilityDlcs =
		customerNormalStore.availableIngredientAvailabilityDlcs.use();
	const availableIngredientLevels =
		customerNormalStore.availableIngredientLevels.use();
	const availableIngredientTags =
		customerNormalStore.availableIngredientTags.use();

	const ingredientPinyinSortState =
		customerNormalStore.persistence.ingredient.pinyinSortState.use();

	const ingredientFilterAvailabilityDlcs =
		customerNormalStore.persistence.ingredient.filters.availabilityDlcs.use();
	const ingredientFilterTags =
		customerNormalStore.persistence.ingredient.filters.tags.use();
	const ingredientFilterNoTags =
		customerNormalStore.persistence.ingredient.filters.noTags.use();
	const ingredientFilterLevels =
		customerNormalStore.persistence.ingredient.filters.levels.use();

	const ingredientPinyinSortConfig = useMemo<IPinyinSortConfig>(
		() => ({
			pinyinSortState: ingredientPinyinSortState,
			setPinyinSortState:
				customerNormalStore.persistence.ingredient.pinyinSortState.set,
		}),
		...
		() => [
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.ingredient.filters
						.availabilityDlcs.set,
				valueType: 'dlc',
			},
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.ingredient.filters.tags.set,
			},
			{
				items: availableIngredientTags,
				label: '食材标签（排除）',
				...
				setSelectedKeys:
					customerNormalStore.persistence.ingredient.filters.noTags
						.set,
			},
			{
				...
				setSelectedKeys:
					customerNormalStore.persistence.ingredient.filters.levels
						.set,
			},
		],
		[
	...
	const ingredientTabVisibilityState =
		customerNormalStore.persistence.ingredient.tabVisibility.use();

	const ingredientTabStyle =
		ingredientTabStyleMap[ingredientTabVisibilityState];

	const isIngredientTabFilterVisible =
		customerNormalStore.shared.ingredient.filterVisibility.use();

	const selectedTabKey = customerNormalStore.shared.tab.use();

	const onTabSelectionChange = useCallback(
		(key: Key) => {
			const nextTabKey = key as TTab;
			vibrate(key !== selectedTabKey);
			customerNormalStore.onTabSelectionChange(nextTabKey);
		},
			...
			{isShowTachie &&
				...
				currentCustomerName !== null && (
					<Tachie
						aria-hidden
						alt="雀酒屋工作装"
						src={getClothesTachiePath('雀酒屋工作装')}
						width={120}
