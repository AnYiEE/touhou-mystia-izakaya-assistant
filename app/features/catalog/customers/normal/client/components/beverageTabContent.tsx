...
export default function BeverageTabContent() {
	...
	const selectedAvailabilityDlcs =
		customerNormalStore.beverageTableAvailabilityDlcs.use();

	const availableBeverageAvailabilityDlcs =
		customerNormalStore.availableBeverageAvailabilityDlcs.use();
	const availableBeverageNames =
		customerNormalStore.availableBeverageNames.use();
	const availableBeverageTags =
		customerNormalStore.availableBeverageTags.use();

	const searchValue = customerNormalStore.shared.beverage.searchValue.use();

	const tableCurrentPage =
		customerNormalStore.shared.beverage.table.page.use();
	const tableRowsPerPage =
		customerNormalStore.shared.beverage.table.rows.use();
	const tableSelectableRows =
		customerNormalStore.shared.beverage.table.selectableRows.get();
	const tableSortDescriptor =
		customerNormalStore.persistence.beverage.table.sortDescriptor.use();
	const tableVisibleColumns =
		customerNormalStore.shared.beverage.table.columns.use();

	const {
		pagedRows: tableCurrentPageItems,
		sortedRows: tableSortedRows,
		totalPages: tableTotalPages,
	} = customerNormalStore.beverageTableRows.use();
						...
						<div className="flex justify-center">
							...
							>
								<Button
									...
									onPress={() => {
										vibrate();
										customerNormalStore.onBeverageTableAction(
											name
										);
									}}
									aria-label={label}
				...
				<div className="flex flex-col justify-between gap-x-3 gap-y-2 md:flex-row">
					<div className="flex flex-1 items-end gap-3">
						<Autocomplete
							...
							onInputChange={(value) => {
								vibrate(!value);
								customerNormalStore.onBeverageTableSearchValueChange(
									value
								);
							}}
						...
						<Select
							...
							variant="flat"
							onSelectionChange={
								customerNormalStore.onBeverageTableSelectedTagsChange
							}
						...
						{availableBeverageAvailabilityDlcs.length > 1 && (
							<Dropdown showArrow>
								...
								<DropdownMenu
									...
									variant="flat"
									onSelectionChange={
										customerNormalStore.onBeverageTableSelectedAvailabilityDlcsChange
									}
	...
	return (
		<BeverageTableShell
			...
			onPageChange={(page) => {
				vibrate();
				customerNormalStore.onBeverageTablePageChange(page);
			}}
			onSortChange={(config) => {
				vibrate();
				customerNormalStore.onBeverageTableSortChange(
