...
function createSearchExpression(searchQuery: string) {
	const escapedSearchQuery = escapeSqliteLikePattern(searchQuery);
...
export async function listAnnouncements({
	...
	return {
		activeCount: normalizeDatabaseCount(
			activeCountRecord.active_count,
			'invalid-announcement-count'
		),
		announcements,
		archivedCount: normalizeDatabaseCount(
			archivedCountRecord.archived_count,
			'invalid-announcement-count'
		),
		filteredCount: normalizeDatabaseCount(
			filteredCountRecord.filtered_count,
			'invalid-announcement-count'
		),
