...
} from './persistence/backupRepository';

export interface ILegacyBackupCleanupResult {
	...
	orphanFoundCount: number;
	temporaryDeletedCount: number;
	temporaryFoundCount: number;
}

export type TLegacyBackupCleanupResult =
	| { data: ILegacyBackupCleanupResult; status: 'ok' }
	| {
			error: 'backup-code-lock-lost' | 'backup-code-lock-timeout';
			status: 'error';
	  };
...
export async function cleanupExpiredLegacyBackups(
	now = Date.now()
): Promise<TLegacyBackupCleanupResult> {
