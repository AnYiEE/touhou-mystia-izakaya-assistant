...
async function acquireSharedBackupCodeLock(
	database: Kysely<TDatabase>,
	code: string,
	ownerId: string
) {
	...
	for (;;) {
		if (await tryAcquireSharedBackupCodeLock(database, code, ownerId)) {
			return;
		}
...
async function renewSharedBackupCodeLock(
	database: Kysely<TDatabase>,
	code: string,
	ownerId: string
) {
...
async function releaseSharedBackupCodeLock(
	database: Kysely<TDatabase>,
	code: string,
	ownerId: string
) {
...
export async function withBackupCodeLock<T>(
	...
	try {
		...
		try {
			await acquireSharedBackupCodeLock(database, code, ownerId);
			...
			try {
				await releaseSharedBackupCodeLock(database, code, ownerId);
			} catch (error) {
