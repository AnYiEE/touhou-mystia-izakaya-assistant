	...
	function AdminSsoCallbackRow({
		...
		return (
			<AdminTableRow>
				...
				<AdminTableCell>
					<AdminMetadata metadata={callback.metadata} />
				</AdminTableCell>
				<AdminTableCell isNowrap>{callback.attempts}</AdminTableCell>
				<AdminTableCell isNowrap>
					{createAdminDateTimeText(callback.next_retry_at)}
				</AdminTableCell>
				<AdminTableCell isNowrap>
					{createAdminDateTimeText(callback.created_at)}
				</AdminTableCell>
...
export default function AdminSsoCallbacksClient({
	...
	const [pageInput, setPageInput] = useState(
		createAdminPageInputValue(initialData.callbacks?.page ?? 1)
	);
	...
	useEffect(() => {
		...
		const startTime = parseAdminTimeInputValue(startTimeInput);
		const endTime = parseAdminTimeInputValue(endTimeInput);
		const nextHref = createAdminHref('/admin/sso/callbacks', {
	...
	const handlePageJumpSubmit = useCallback(
		(event: SyntheticEvent<HTMLFormElement>) => {
			...
			setPage(
				parseAdminPageInput(pageInput, callbacks?.total_pages ?? 1)
			);
		},
