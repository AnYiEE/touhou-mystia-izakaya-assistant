...
import { getAccountCookieSecureFlag } from '@/features/account/server/auth/cookiePolicy';
import {
	createAccountCookieDomainOptions,
	getAccountCookieDomain,
} from '@/features/account/server/auth/session';
...
function resolveRpID() {
	...
	if (cookieDomain === undefined) {
		return new URL(PUBLIC_RUNTIME_CONFIG.baseOrigin).hostname;
	}
...
export function getWebAuthnRelyingParty(): IWebAuthnRelyingParty {
	...
	return { expectedOrigin, rpID, rpName: SITE_METADATA.name };
}
