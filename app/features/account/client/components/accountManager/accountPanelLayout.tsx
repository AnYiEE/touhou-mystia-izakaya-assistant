...
const ACCOUNT_COLLAPSE_SIZE_MOTION_ANIMATE = { height: 'auto' } as const;
const ACCOUNT_COLLAPSE_SIZE_MOTION_HIDDEN = { height: 0 } as const;
const ACCOUNT_COLLAPSE_MOTION_STYLE = { overflow: 'hidden' } as const;
...
export const AccountCollapseMotion = memo<IAccountCollapseMotionProps>(
	function AccountCollapseMotion({
		children,
		className,
		motionKey,
		...
		const animate = shouldAnimateOpacity
			? ACCOUNT_COLLAPSE_MOTION_ANIMATE
			: ACCOUNT_COLLAPSE_SIZE_MOTION_ANIMATE;
		const hidden = shouldAnimateOpacity
			? ACCOUNT_COLLAPSE_MOTION_HIDDEN
			: ACCOUNT_COLLAPSE_SIZE_MOTION_HIDDEN;
				...
				{shouldRender ? (
					isReducedMotion ? (
						...
						<motion.div
							...
							exit={hidden}
							initial={hidden}
							style={ACCOUNT_COLLAPSE_MOTION_STYLE}
	...
	function AccountAuthEntryMotion({ children, motionKey }) {
		...
		return (
			<AnimatePresence initial={false}>
				<motion.div
					key={motionKey}
					animate={ACCOUNT_COLLAPSE_MOTION_ANIMATE}
					exit={ACCOUNT_COLLAPSE_MOTION_HIDDEN}
					initial={ACCOUNT_COLLAPSE_MOTION_HIDDEN}
					style={ACCOUNT_COLLAPSE_MOTION_STYLE}
