...
import type { IAccountSyncBroadcastMessage } from '@/features/account/sync/types';
...
export function getAccountSyncBroadcastChannel() {
	try {
		...
		console.warn('Account sync broadcast channel is unavailable.', {
...
export function postAccountSyncBroadcastMessage(
	...
	try {
		return broadcastChannel
			...
			.then(() => true)
			.catch((error: unknown) => {
				console.warn('Account sync broadcast failed.', {
					errorCode: getLogSafeErrorCode(error),
				});
				return false;
			});
...
export function subscribeAccountSyncBroadcastMessage(
	...
	try {
		...
		console.warn('Account sync broadcast subscription failed.', {
			errorCode: getLogSafeErrorCode(error),
		});
		return () => {};
	}
	...
	return () => {
		try {
			...
			console.warn('Account sync broadcast unsubscription failed.', {
				errorCode: getLogSafeErrorCode(error),
			});
		}
	};
...
function getAccountWebauthnBroadcastChannel() {
	try {
		...
		console.warn('Account WebAuthn broadcast channel is unavailable.', {
			errorCode: getLogSafeErrorCode(error),
		});
		return null;
	}
...
export function postAccountWebauthnBroadcastMessage(
	...
	try {
		return broadcastChannel
			...
			.then(() => true)
			.catch((error: unknown) => {
				console.warn('Account WebAuthn broadcast failed.', {
					errorCode: getLogSafeErrorCode(error),
				});
				return false;
			});
...
export function subscribeAccountWebauthnBroadcastMessage(
	...
	try {
		...
		console.warn('Account WebAuthn broadcast subscription failed.', {
			errorCode: getLogSafeErrorCode(error),
		});
		return () => {};
	}
	...
	return () => {
		try {
			...
			console.warn('Account WebAuthn broadcast unsubscription failed.', {
				errorCode: getLogSafeErrorCode(error),
			});
		}
	};
