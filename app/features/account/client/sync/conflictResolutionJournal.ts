...
export function parseAccountSyncConflictResolutionJournalValue({
	...
	if (
		isNonNegativeSafeInteger(parsed['version']) &&
	...
	if (
		...
		(resultBaseRevision !== null &&
			!isNonNegativeSafeInteger(resultBaseRevision)) ||
		...
		(resultSchemaVersion !== null &&
			!isNonNegativeSafeInteger(resultSchemaVersion)) ||
		typeof stage !== 'string' ||
		...
		return {
			status: 'invalid',
			version: isNonNegativeSafeInteger(version) ? version : null,
		};
	}
