...
import {
	readCustomerRarePlansPersistenceSnapshot,
	replaceCustomerRarePlansPersistenceSnapshot,
} from '@/features/customerPlans/client/state/accountSync';
...
const defaultSnapshot: ICustomerRarePlansState = { activeId: null, items: [] };
...
function normalizeCustomerRarePlansSnapshot(data: ICustomerRarePlansState) {
	...
	return { activeId, items } satisfies ICustomerRarePlansState;
}
...
export const customerRarePlansSerializer = {
	...
	validate(data): data is ICustomerRarePlansState {
		if (
			!isObjectTagRecord(data) ||
...
} satisfies ISyncNamespaceSerializer<ICustomerRarePlansState>;
