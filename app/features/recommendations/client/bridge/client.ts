...
import { suggestMeals } from '@/features/recommendations/client/suggestMeals';
...
function checkAccountGate(target: IBridgeRuntime) {
	const { isAuthenticated, isBootstrapComplete } =
		target.accountGate.getSnapshot();

	return isBootstrapComplete && isAuthenticated;
}
...
function scheduleGateCheck(target: IBridgeRuntime) {
	...
	scheduleMicrotask(() => {
		...
		if (target.accountGate.getSnapshot().isDisabled) {
			stopRuntime(target);
			return;
		}
		if (!checkAccountGate(target)) {
...
function createRuntime(accountGate: IAccountGatePort): IBridgeRuntime {
	return {
		accountGate,
...
export function startRecommendationBridgeClient(accountGate: IAccountGatePort) {
	startCount++;
	cleanupGeneration++;
	if (
		runtime === null &&
		...
		const target = createRuntime(accountGate);
		runtime = target;
		const stopAccountGateSubscription = accountGate.subscribe(() => {
			scheduleGateCheck(target);
		});
		const stopVersionSubscription =
			recommendationPreferencesFacade.version.onChange((version) => {
				if (
					version !== null &&
					compareVersions(version, SITE_METADATA.version) > 0
				) {
					const generation = target.connectionGeneration;
					sendForGeneration(target, generation, {
		...
		target.stopSubscriptions.push(
			stopAccountGateSubscription,
			stopVersionSubscription
		);
		const handleOnline = () => {
			if (
