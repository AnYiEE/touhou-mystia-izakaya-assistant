	...
	},
	{
		...
		spellCards: {
			...
			negative: [
				{
					name: '想起「那年的素兔的恐怖」',
					description: `再一次激活强买强卖功能，如果不买，将发动另一张符卡“恶作剧「Combo消失术」”。${LABEL_BR}恶作剧「Combo消失术」${LABEL_BR}中断Combo，且在30秒内无法再获得Combo。`,
				},
			],
		},
		positiveTagMapping: {
	...
	},
	{
		...
		spellCards: {
			positive: [
				{
					name: '「八衢潮流风靡幻想」',
					description: `明天开始店铺变为“明星店”，三天后恢复正常。“明星店”：提高10%顾客预算和5%客流量，“招牌”标签成为流行标签。如果店铺已是“明星店”或当晚已宣传，则此符卡转化为“「食堂风起厨神无双」”。${LABEL_BR}「食堂风起厨神无双」${LABEL_BR}成品带有“招牌”标签的今日菜单上的料理将成为明星料理。制作明星料理时，如果料理时间小于5秒，则瞬间完成；在舆论的裹挟下，追捧“${TAG_POPULAR_POSITIVE}”标签的顾客食用明星料理后额外提高30%续单率，返还当单酒水消耗的预算，并必定增加一次续单上限（增加续单上限效果每桌顾客只触发一次），持续30秒。`,
				},
			],
			negative: [
				{
			...
			positive: [
				{
					name: '「花鸟风月群芳沐」',
					description:
						'在场上种下一朵鲜花并利用能力使其生长，满心情时额外种下一朵，每朵生长的鲜花每24秒生产25小费。除幽香以外的稀有顾客释放奖励符卡时，随机一朵生长的鲜花将吸收符卡之力而盛放，吸引当地出没的稀有顾客造访食堂，被鲜花吸引来的稀有顾客会摘走这朵鲜花。鲜花盛放期间顾客用餐结束后额外提高15心情，15秒后鲜花凋谢，获得一层可以抵挡一次中断Combo的失误的“落英缀裳”。',
				},
			],
			negative: [
				{
	...
	},
	{
		...
		spellCards: {
			positive: [
				{
					name: '欺符「超限效应」',
					description: `令全场除自身以外的顾客的厌恶标签反转为喜好标签，持续30秒。当场上包括排队顾客在内的普通顾客数量之和不少于7时，此符卡转化为“逆弓「天壤梦弓的诏敕」”${LABEL_BR}逆弓「天壤梦弓的诏敕」${LABEL_BR}使场上包括排队顾客在内的普通顾客额外获得一次续单上限和25%预算，每位普通顾客最多可以受到此效果影响三次。`,
				},
			],
			negative: [
				{
	...
	},
	{
		...
		spellCards: {
			positive: [
				{
					name: '「幽冥的轮回」',
					description: `如果场上不存在“幽玄封魔阵”则生成法阵，如果存在则为法阵填充2点能量。“幽玄封魔阵”：除魅魔以外的稀有顾客释放任意符卡时填充1点能量。能量达到7点时，消耗全部能量引发“梦符「时空一粟」”${LABEL_BR}梦符「时空一粟」${LABEL_BR}场上所有就座的顾客的剩余预算回复到上限，制作料理不消耗食材且瞬间完成，顾客必定给出最高评价，持续20秒。`,
				},
			],
			negative: [
				{
	...
	},
	{
		...
		spellCards: {
			positive: [
				{
					name: '「食物瞬间消失术」',
					description: `接下来直到营业结束为止，顾客会瞬间吃下所有料理。再次触发时此符卡转化为“「墨染浮櫻化醉蝶」”。${LABEL_BR}「墨染浮櫻化醉蝶」${LABEL_BR}幽幽子释放1/3/5/8（随使用次数提升）只蝴蝶飞向保温箱，每只蝴蝶吞噬保温箱中存储的一个料理，转化为等同于四倍料理价格的金钱（享受小费倍率加成）。`,
				},
			],
			negative: [
				{
