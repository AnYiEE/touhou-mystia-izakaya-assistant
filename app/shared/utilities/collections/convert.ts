...
function isIterable<T>(
	value: T | Iterable<T>
): value is Extract<T, Iterable<T>> | Iterable<T> {
	return (
...
export function toArray<T>(...args: T[]) {
	if (args.length === 1) {
		// The length guard proves that the first argument exists.
...
export function copyArray<T>(array: ReadonlyArray<T>) {
	return [...array];
}
