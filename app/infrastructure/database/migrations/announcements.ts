...
import {
	ANNOUNCEMENT_DEFAULT_AUDIENCE,
	ANNOUNCEMENT_DEFAULT_LEVEL,
	ANNOUNCEMENT_VERSION_DEFAULT_ACTION,
...
type TAnnouncementColumnDefinition = ISqliteMigrationColumnDefinition & {
	structural?: boolean;
};
...
const ANNOUNCEMENT_TABLE_COLUMN_DEFINITION_MAP = {
	[TABLE_NAME_MAP.announcement]: {
		audience: {
			dataType: 'text',
			defaultTo: ANNOUNCEMENT_DEFAULT_AUDIENCE,
			notNull: true,
		},
	...
	[TABLE_NAME_MAP.announcementVersion]: {
		action: {
			dataType: 'text',
			defaultTo: ANNOUNCEMENT_VERSION_DEFAULT_ACTION,
			notNull: true,
		},
		announcement_id: { dataType: 'text', structural: true },
	...
	Record<string, TAnnouncementColumnDefinition>
>;
...
async function ensureTableColumns(
	...
	const columnDefinitionMap = ANNOUNCEMENT_TABLE_COLUMN_DEFINITION_MAP[
		tableName
	] as Record<string, TAnnouncementColumnDefinition>;
...
export async function migrateAnnouncementTables(database: Kysely<TDatabase>) {
	await database.schema
		...
		.addColumn('level', 'text', (col) =>
			col.notNull().defaultTo(ANNOUNCEMENT_DEFAULT_LEVEL)
		)
		.addColumn('audience', 'text', (col) =>
			col.notNull().defaultTo(ANNOUNCEMENT_DEFAULT_AUDIENCE)
		)
