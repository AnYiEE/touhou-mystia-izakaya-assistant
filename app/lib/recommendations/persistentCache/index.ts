...
import {
	getRecommendationCacheStoreStats,
...
export function getPersistentRecommendationCacheStats() {
	return Promise.all([
		getRecommendationCacheStoreStats('suggestedMealCardResults'),
		getRecommendationCacheStoreStats('customerRarePlanResults'),
	]).then(([suggestedMealCardResults, customerRarePlanResults]) => ({
		customerRarePlanResults,
		suggestedMealCardResults,
	}));
}

export { deleteCurrentRecommendationCacheDatabase } from './database';
